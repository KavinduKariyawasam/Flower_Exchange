...
public:
    ...
    vector<Order_Book*> Matching(Order* order, Execution*& transaction, vector<Order_Book*>& Buy_Book, vector<Order_Book*>& Sell_Book){
        ...
        for (Order* order : orders) {
            ...
            if (is_Invalid_Order(order, transaction)){
                ...
                if (order->Instrument == "Rose") {
                    Buy_Rose, Sell_Rose = Matching(order, transaction, Buy_Rose, Sell_Rose);
                }
                else if (order->Instrument == "Lavender") {
                    Buy_Lavender, Sell_Lavender = Matching(order, transaction, Buy_Lavender, Sell_Lavender);
                }
                else if (order->Instrument == "Lotus") {
                    Buy_Lotus, Sell_Lotus = Matching(order, transaction, Buy_Lotus, Sell_Lotus);
                }
                else if (order->Instrument == "Tulip") {
                    Buy_Tulip, Sell_Tulip = Matching(order, transaction, Buy_Tulip, Sell_Tulip);
                }
                else if (order->Instrument == "Orchid") {
                    Buy_Orchid, Sell_Orchid = Matching(order, transaction, Buy_Orchid, Sell_Orchid);
                }
            }
            i++;
